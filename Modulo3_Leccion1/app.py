...
from flask_principal import Principal, Permission, RoleNeed, Identity, identity_changed, identity_loaded, UserNeed, AnonymousIdentity
import json
...
def login():
    ...
    if form.validate_on_submit():
        ...
        for user in users:
            if user["username"] == username and check_password_hash(user["password"], password):  # Usar check_password_hash
        ...
        for admin in admins:
            if admin["username"] == username and check_password_hash(admin["password"], password):  # Usar check_password_hash
...
@login_required
def home():
    return render_template("home.html", message=f"Bienvenido, {current_user.id}. Rol: {current_user.role}")
...
def forbidden(e):
    return "<h1>403 - Acceso Prohibido</h1><p>No tienes permiso para ver esta página.</p>", 403
